...
import (
	"goframework/internal/api/admin"
	"goframework/internal/api/authorized"
	"goframework/internal/api/config"
	"goframework/internal/api/helper"
	"goframework/internal/api/menu"
	"goframework/internal/api/tool"
...
func setApiRouter(r *resource) {
	...
	api := r.mux.Group("/api", core.WrapAuthHandler(r.interceptors.CheckLogin), r.interceptors.CheckSignature(), r.interceptors.CheckRBAC())
	{
		...
		// config
		configHandler := config.New(r.logger)
